...
/**
 * 用本地 45 位填报器内容一次性覆盖云端“当前方案草稿”。
 * 不能循环调单条加入接口，否则只会保留最后一次待确认项。
 */
async function replaceCurrentPlanDraftItems(items) {
  const previousItems = currentPlanItems.value;
  const previousSourceType = latestSourceType.value;
  const previousSourceQuery = latestSourceQuery.value;
  const nextItems = [];
  (items || []).forEach((item) => {
    const normalized = normalizeItem(item, item?.strategy);
    const planKey = buildPlanItemKey(normalized, normalized.strategy);
    if (nextItems.some((entry) => entry.planKey === planKey)) return;
    nextItems.push({ ...normalized, planKey });
  });
  currentPlanItems.value = nextItems;
  latestSourceType.value = "score";
  latestSourceQuery.value = `本地填报器同步 ${nextItems.length} 条志愿结果`;
  try {
    await upsertCurrentPlanDraft();
    return nextItems.length;
  } catch (error) {
    currentPlanItems.value = previousItems;
    latestSourceType.value = previousSourceType;
    latestSourceQuery.value = previousSourceQuery;
    throw error;
  }
}
